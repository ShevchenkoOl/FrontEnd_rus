...
Вот твой текст, оформленный в Markdown для конспекта. Я сохранил оригинальную структуру и добавил визуальные акценты для лучшей читаемости.
...
```
Получается **«многослойный пирог»**, в котором первая тень в списке размещается на самом верху, последняя — в самом низу. То есть, чем раньше указана тень, тем выше она в слоях и будет визуально перекрывать все последующие.

> **💡 Преимущество:** Этот подход позволяет сделать очень мягкие и красивые тени.

Если к каждой тени добавить разные цвета, то получится **разноцветная тень**:
...
* **[Squoosh](https://squoosh.app/)** — продвинутый инструмент от Google. Позволяет вручную настраивать степень сжатия и сразу видеть результат «до и после» в реальном времени.
