# Лекция 14: Роутинг (Маршрутизация)
...
document.addEventListener("DOMContentLoaded", () => {  
    // Вешаем слушатель на body, чтобы ловить клики динамических элементов, DOMContentLoaded означает: "Подожди, пока браузер полностью прочитает весь HTML и создаст тег <body>, и только потом запускай код внутри". Это стандартная мера безопасности. Если мы попытаемся повесить клик на document.body до того, как этот body появится, код выдаст ошибку.
