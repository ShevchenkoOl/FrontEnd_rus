# Лекция 7: Callback и дополнительные методы массивов (forEach, map, filter, sort, reduce)
...
### 2.5 some() и every() — Проверки

  * `some()` — хотя бы один элемент подходит? (Вернет `true`/`false`).
```javascript
   const numbers = [1, 3, 5, 8];
  const hasEven = numbers.some(num => num % 2 === 0);
  console.log(hasEven); // true, потому что 8 — чётное
```
  * `every()` — **все** элементы подходят? Возвращает `true`, если каждый элемент подходит и возвращает `false`, если хотя бы один элемент не подходит
```javascript
   const numbers = [2, 4, 6, 8];
   const allEven = numbers.every(num => num % 2 === 0);
   console.log(allEven); // true

   const numbers2 = [2, 3, 6, 8];
   console.log(numbers2.every(num => num % 2 === 0)); // false
```

### 2.6 sort() — Сортировка
Используется, чтобы отсортировать элементы массива.
...
### 2.7 reduce() — Агрегация
reduce() позволяет «свернуть» массив в одно значение, применяя колбек ко всем элементам по порядку.
  - Возвращает одно значение любого типа: число, объект, массив, строку…
  - Можно использовать для суммирования, подсчёта, объединения, группировки.
...
-----
## 3. Контекст `this`
Можно с уверенностью сказать, что ключевое слово `this` — это одна из самых запутанных концепций JavaScript на начальном этапе обучения. Контекст в JavaScript похож на контекст в предложении:<br>
```
Петя бежит быстро, потому что Петя пытается успеть на поезд.
Петя бежит быстро, потому что он пытается успеть на поезд.
```
Второе предложение звучит более лаконично. Подлежащее предложения — **Петя**, и мы можем сказать, что контекст предложения — это **Петя**, потому что он находится в центре внимания в данный момент в предложении. Даже местоимение **«он»** относится тоже к Пете.
И точно так же объект может быть текущим контекстом выполнения функции.
```javascript
const petya = {
  username: "Petya",
  showName() {
...
petya.showName();
```
Обращение к свойствам объекта внутри методов с использованием имени самого объекта аналогично использованию «Петя» вместо «он».
Внутри функций можно использовать зарезервированное ключевое слово **this**. Во время выполнения функции в `this` записывается ссылка на объект, в контексте которого она была вызвана. Таким образом, в теле функции мы можем получить доступ к свойствам и методам этого объекта.
```javascript
cconst petya = {
  username: "Petya",
  showName() {
...
const user = {
    name: "Alex",
    age: 25,
    sayHi() { // Это обычная функция (метод объекта), которая вызываеться в контексте обьекта user
