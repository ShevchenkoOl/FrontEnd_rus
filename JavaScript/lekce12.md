...
6. **[Axios Docs](https://www.google.com/search?q=https://axios-http.com/ru/docs/intro)** — Официальное руководство по Axios. Там можно подсмотреть, как делать более сложные запросы (например, как передавать заголовки авторизации).
7. **[JSON Server (GitHub)](https://github.com/typicode/json-server)** — Полная инструкция по локальному фейковому бэкенду. Там описано, как делать сортировку, фильтрацию и поиск по твоему `db.json`.
8. **[Faker.js API](https://fakerjs.dev/api/)** — Официальная документация генератора фейковых данных (который используется внутри MockAPI). Здесь можно найти точные названия всех методов (например, как сгенерировать фейковый email или номер кредитки).
